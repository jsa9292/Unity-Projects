...
namespace com.rfilkov.components
{
    ...
    public class JointOverlayer : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            //if (sensorTransform == null) sensorTransform = DetectPilot.instance.cell_control_transform;
            //if (!foregroundCamera)
            //{
            //    // by default - the main camera
            //    foregroundCamera = Camera.main;
            //}
            if (!sensorTransform) sensorTransform = kinectManager.transform;
        ...
        void Update()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (trackedJointInt >= 0) iJointIndex = trackedJointInt;
                if (kinectManager.IsJointTracked(userId, 1))
                {
                    ...
                    if (posJoint != Vector3.zero && overlayObject)
                    {
                        ...
                        Vector3 delta_pos = (posJoint - overlayObject.position);
                        if (delta_pos.magnitude > pos_thres) overlayObject.position = overlayObject.position + delta_pos;
                        else overlayObject.position = overlayObject.position + delta_pos * smoothing;
